"""
Live check of HLAdapter.search() against the real Hapag-Lloyd SPA - not
part of the automated test suite. Run manually to confirm the adapter
still works.

Run: python -m tracking_control_tower.tests.run_hl_live_check <container>
"""
import sys
...
def main() -> None:
    container = sys.argv[1] if len(sys.argv) > 1 else "FCIU4746425"
    print(f"Searching HL for container: {container}")

    with HLAdapter(headless=False) as adapter:
